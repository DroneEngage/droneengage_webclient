...
const CMDTYPE_COMM 			= 'c';   		// communication command
...
const CMD_SYS_TASKS 		='tsk';   		// group broadcast
...
var SOCKET_STATUS = ['Fresh','Connecting','Disconnecting','Disconnected','Connected','Registered','UnRegistered','Error'];
...
(function (global){
    ...
    var AndruavClient = function () {

		var Me = this;
		var v_waypointsCache = {};
		var v_callbackListeners = {};
		
		var _fn_checkStatus = function()
		{
			...
			var now = Date.now();
			var len = Me.m_andruavUnitList.fn_getUnitCount();
			var arr = Me.m_andruavUnitList.fn_getUnitValues();
			var v_unit;
			for (var i =0 ; i<len; ++i )
			{
				v_unit = arr[i];
				...
				{	
					...
					{
						Me.API_requestID (v_unit.partyID);
					}
					else
					{
						if (v_unit.m_IsShutdown == false)
						{	// received a message from a v_unit that where marked off
		...
		this.fn_init = function ()
        {
			...
			*/
			...
			this.m_andruavUnitList = new AndruavUnitList();

			if (this.fn_timerID_checkStatus == null)
            {
            
                this.fn_timerID_checkStatus = setInterval(function () {_fn_checkStatus()} , CONST_checkStatus_Interverl0);
			}
			...
			/////////////////////////////////////////////////////////////////////
			// EVENT HANDLER AREA
			var _fn_sendRXChannels = function ()
			{
				if (v_sendAxes === false) return ;
					
				v_sendAxes = false;
				p_me.API_sendRXChannels(v_axes);
			}
			
			if (this.fn_timerID_sendRXChannels == null)
			{
				this.fn_timerID_sendRXChannels = setInterval (_fn_sendRXChannels, CONST_sendRXChannels_Interval);
			}

			// receives event from gamepad and store it for sending.
			function fn_sendAxes (p_me)
			{
				// game pad should be attached to a unit.
				if (p_me.m_gamePadUnit == null) return ;

				const c_controller = window.AndruavLibs.AndruavGamePad.fn_getGamePad(0);
				if (c_controller == null) return ;
				// read gamepad values
				v_axes = c_controller.p_axes;
				v_sendAxes = true;

				fn_console_log ("fn_sendAxes");
			}

			function fn_sendButtons (p_me, p_packet)
			{
				// game pad should be attached to a unit.
				if (p_me.m_gamePadUnit==null) return ;
			...
			function fn_requestGamePad (p_me, p_andruavUnit)
			{
				p_me.m_gamePadUnit = p_andruavUnit;
			}

			function fn_releaseGamePad (p_me, p_andruavUnit)
			{
				p_me.m_gamePadUnit = null;
			}

			window.AndruavLibs.EventEmitter.fn_subscribe(EE_GamePad_Axes_Updated,this, fn_sendAxes);
			window.AndruavLibs.EventEmitter.fn_subscribe(EE_GamePad_Button_Updated,this, fn_sendButtons);
			window.AndruavLibs.EventEmitter.fn_subscribe(EE_requestGamePad,this, fn_requestGamePad);
			window.AndruavLibs.EventEmitter.fn_subscribe(EE_releaseGamePad,this, fn_releaseGamePad);
		...
		*/
		this.fn_callbackOnMessageID = function (p_callback, p_messageID)
		{
			v_callbackListeners[p_messageID] = p_callback;	
		}

		this.API_addMe2 = function ()
		{
			if ((this.partyID == null) || (this.m_groupName == null)) return ;
			
			var v_unit = new AndruavUnitObject();
			v_unit.m_IsMe 					= true;
			v_unit.m_IsGCS 					= true;
		...
		this.API_sendCMD = function (p_target,msgType,msg)
		{
			var v_rountingMsg;
			if (p_target != null)
			{
			...
			else
			{
				...
				// if you want to prevent GCS to GCS.
		...
		this.API_sendBinCMD = function (targetName,msgType,msg,data)
		{
			var v_msgRouting;
			if (targetName != null)
			{
			...
			reader.onload = function(event) {
										...
										msgx = fn_concatBuffers (h,contents,true);
										ws.sendex (msgx,true);
			};
		...
		this._API_sendSYSCMD = function (p_msgID, p_msg)
		{
			if (this.ws!= null)
			{			
		...
		this.API_saveGeoFenceTasks = function (accountID,m_groupName,p_partyID,receiver,isPermanent,geofenceInfo)
		{
			if (p_partyID==null) p_partyID = '_any_';
			if (m_groupName==null) m_groupName = '_any_';
			const c_msg = 
			{
				ai: accountID,
				ps: p_partyID,
		...
		this.API_loadGeoFence = function (accountID,m_groupName,p_partyID,receiver,isPermanent)
		{
			if (p_partyID==null) p_partyID = '_any_';
			if (m_groupName==null) m_groupName = '_any_';
			const c_msg = 
		...
		this.API_disableGeoFenceTasks = function (p_accountID,m_groupName,p_partyID,receiver,isPermanent)
		{
			if (p_partyID==null) p_partyID = '_any_';
			if (m_groupName==null) m_groupName = '_any_';
		 ...
		 */
		this.API_engageRX  = function (p_andruavUnit)
        {
            if ( (this.m_currentEngagedRX!= null) && (this.m_currentEngagedRX.partyID !=  p_andruavUnit.partyID))	
            {	// This webGCS is already engaged with another Drone. so Tell Drone I am no longer controlling you.
		 ...
		 */
        this.API_disengageRX  = function (p_andruavUnit)
        {
		...
		this.API_engageGamePad = function (p_andruavUnit)
		{
			let p_msg = {
                'b':CONST_RC_SUB_ACTION_JOYSTICK_CHANNELS  //rcSubAction 
                //'b':CONST_RC_SUB_ACTION_JOYSTICK_CHANNELS_GUIDED  
            };
            p_andruavUnit.m_Telemetry.m_rxEngaged = true;
            this.m_currentEngagedRX = p_andruavUnit; // it might be already null and not synch-ed
		...
		this.API_requestIMU = function (p_andruavUnit, on_off)
		{
		...
		this.API_startTelemetry  = function (p_andruavUnit,lvl)
		{
			...
			window.AndruavLibs.EventEmitter.fn_dispatch(EE_servoOutputUpdate,p_andruavUnit);
		};
		
		
		this.API_resumeTelemetry  = function (lvl)
		{
			if (this.currentTelemetryUnit == null) return;
		...
		this.API_stopTelemetry  = function (p_andruavUnit)
		{
		...
		this.API_SendTelemetryData = function (p_andruavUnit,data)
		{
			//var msg = {};
			//msg.src = CONST_TELEMETRY_SOURCE_GCS; 
			
			this.API_sendBinCMD (p_andruavUnit.partyID,CONST_TYPE_AndruavMessage_LightTelemetry,null,data);
		
		};
		
		
		
		this.API_sendID  = function (p_target)
		{
			var msg = 
			{
		...
		this.API_requestID  = function (p_partyID)
		{
			var msg = 
			{
		...
		this.API_sendRXChannels  = function (p_axes)
        {
			var v_axis = [0,1,2,3];
			if ((this.m_gamePadUnit == null) || (this.m_gamePadUnit.partyID == null)) return ;
		 ...
		 * 
		 */
		this.API_do_ServoChannel = function (p_partyID, p_channelNum, p_value)
		{
		...
		this.API_do_Arm  = function (p_partyID, param_toArm, param_emergencyDisarm) 
		{
			var msg = 
			{
				A:param_toArm,
		...
		this.API_do_ChangeAltitude  = function (p_partyID, param_altitude) 
		{
			var msg = 
			{
		...
		this.API_do_YAW = function (p_partyID, var_targetAngle, var_turnRate, var_isClockwise, var_isRelative)
		{
			var msg = 
			{
		...
		this.API_do_SetHomeLocation = function (p_partyID, p_lat,p_lng,p_alt)
        {
            if (p_alt == null) 
            {
                p_alt = 0;
		...
		this.API_do_GimbalCtrl = function (p_partyID, p_pitch, p_roll, p_yaw, p_isAbsolute)
		{
			var v_msg = 
			{
		...
		this.API_do_ChangeSpeed1 = function (p_partyID, p_speed)
		{
			this.API_do_ChangeSpeed2 (p_partyID, p_speed);
		}  
		
		
		this.API_do_ChangeSpeed2 = function (p_partyID, p_speed, p_isGroundSpeed, p_throttle, p_isRelative)
		{
			var v_msg = 
			{
				a:p_speed,
		...
		this.API_do_Land  = function (p_partyID) 
		{
			var v_msg = 
			{
		...
		this.API_do_FlightMode = function (p_partyID, flightMode)
		{
			var v_msg =
			{
				F: flightMode
		...
		this.API_setGPSSource  = function (p_partyID,p_source) //(p_andruavUnit,OnOff)
		{
		...
		this.API_WebRTC_Signalling = function (p_partyID, p_webrtcMsg)
		{
			var v_msg =
			{
				w: p_webrtcMsg
		...
		this.API_CONST_RemoteCommand_streamVideo  = function (p_andruavUnit, p_OnOff, p_number, p_channel)
		{
		 ...
		 * @param {any} zVel
		 */
		this.API_do_FlyHere = function (p_partyID,p_latitude,p_longitude,p_altitude,p_xVel,p_yVel,p_zVel)
		{
			var v_msg =
			{
				a: p_latitude,
		...
		this.API_do_CircleHere = function (p_partyID,p_latitude,p_longitude,p_altitude,p_radius,p_turns)
		{
			var v_msg =
			{
				a: p_latitude,
		...
		this.API_requestReloadLocalGroupGeoFenceTasks = function (v_target)
		{
			_API_requestReloadLocalGroupTasks (v_target,CONST_TASK_SCOPE_LOCALGROUP,CONST_TYPE_AndruavMessage_ExternalGeoFence);
		}



		// local function
		function _API_requestReloadLocalGroupTasks (v_target,v_taskscope,v_tasktype)
		{
			var v_msg = 
			{
			...
			if (v_tasktype != null) v_msg.tp = v_tasktype;
			
			Me.API_sendCMD (v_target,CONST_TYPE_AndruavMessage_RemoteExecute,v_msg);
		};



		this.API_requestGeoFences  = function (p_andruavUnit, p_fenceName)
		{
			var v_msg = 
				{
		...
		this.API_requestGeoFencesAttachStatus  = function (p_andruavUnit, p_fenceName)
		{
		...
		this.API_requestDeleteGeoFences  = function (p_andruavUnit, p_fenceName)
		{
		...
		this.API_uploadWayPoints  = function (p_andruavUnit,p_eraseFirst,p_textMission)
		{

			
			// eraseFirst NOT IMPLEMENTED YET
			var v_msg = {};
		...
		this.API_clearWayPoints  = function (p_andruavUnit,p_enableFCB)
		{

			var v_msg = {};
			if (p_enableFCB === false) return;
			
			v_msg.C = CONST_RemoteCommand_CLEAR_WAY_POINTS;
			this.API_sendCMD (p_andruavUnit.partyID,CONST_TYPE_AndruavMessage_RemoteExecute, v_msg);
		 ...
		 */
		this.API_do_StartMissionFrom  = function (p_andruavUnit,p_missionNumber)
		{

			if (p_missionNumber < 0 ) p_missionNumber = 0;
			var p_msg = {};
		...
		this.API_requestWayPoints  = function (p_andruavUnit,p_enableFCB)
		{
			...
			if (v_waypointsCache.hasOwnProperty(p_andruavUnit.partyID) === true)
			{	
				...
				delete v_waypointsCache[p_andruavUnit.partyID];
			}
		...
		this.API_requestCameraList  = function (p_PartyID, p_callback)
        {
			if (p_callback != null)
			{
		...
		this.API_requestDeleteWayPoint  = function (p_PartyID, p_fenceName)
        {
        ...
        * */
	   this.API_disableWayPointTasks = function (p_accountID,p_groupName,p_partyID,p_receiver,p_isPermanent)
	   {
		   if (p_partyID==null) p_partyID = '_any_';
		   if (p_groupName==null) p_groupName = '_any_';
		   let p_msg = 
		   {
			   ai: p_accountID,
			   ps: p_partyID,
		...
		this.API_TXCtrl = function (p_andruavUnit,p_subAction)
        {
            var p_msg = 
            {
                b: p_subAction
		...
		this.API_connectToFCB = function (p_andruavUnit)
        {
            var p_msg = 
            {
		...
		this.API_SwitchCamera = function (p_target, p_cameraUniqueName)
		{
			var msg = 
			{
		...
		this.API_TurnMobileFlash = function (p_target, p_flashOn, p_cameraUniqueName)
		{
			var msg = 
			{
				f: p_flashOn,
		...
		this.API_CONST_RemoteCommand_zoomCamera  = function (p_target, p_cameraUniqueName, p_isZoomeIn, p_zoomValue, p_zoomValueStep)
		{
			var msg = 
			{
				u: p_cameraUniqueName,
			...
			if (p_zoomValue != null)  	  msg.b = p_zoomValue;
			if (p_zoomValueStep != null)  msg.c = p_zoomValueStep;
		...
		this.API_CONST_RemoteCommand_takeImage2  = function (p_target,_cameraSource, _numberofImages,_timeBetweenShots, _distanceBetweenShots)
		{
			var msg = 
			{
				a: _cameraSource,
		...
		this.API_CONST_RemoteCommand_recordVideo  = function (p_target, p_trackId, p_OnOff)
		{
			var v_unit = Me.m_andruavUnitList.fn_getUnit(p_target);
			if (v_unit==null) 
			{
		...
		this.fn_disconnect = function (p_accesscode)
        {
		...
		this.fn_connect = function (p_accesscode)
		{

			function _fn_onNewUnitAdded (target)
			{
				//this.API_requestGeoFences (p_andruavUnit);
				Me.API_requestGeoFencesAttachStatus (target);
				
				Me.API_requestWayPoints(target);
				
			};


			var prv_parseFenceInfo = function (p_andruavUnit, p_jmsg)
			{
				var fencetype;
				var shouldKeepOutside = false;
				//var jmsg 				= msg._mg;
						
				var geoFenceName 		= p_jmsg.n;
				var maximumDistance 	= (p_jmsg.hasOwnProperty('r'))?p_jmsg.r:0;   //optional
				if (p_jmsg.hasOwnProperty('o'))
				{ // 1 if restricted area
					shouldKeepOutside  	= (p_jmsg.o==1);   //optional
				}
				...
				{ // 1 if restricted area
					...
					{
						case 1:
							fencetype = CONST_TYPE_LinearFence;
						break;
						
						case 2:
							fencetype = CONST_TYPE_PolygonFence;
						break;
						
						case 3:
							fencetype = CONST_TYPE_CylinderFence;
						break;
					}
				}
				var geoFenceInfo		= {};
				...
				for (var i=0; i< count; ++ i)
				{
					...
					if (p_jmsg[i].hasOwnProperty('l')) lnglat.alt = parseFloat(p_jmsg[i].l); else lnglat.alt  = 0;// altitude
					LngLatPoints.push(lnglat);
				} 
				geoFenceInfo.shouldKeepOutside 	= shouldKeepOutside;
				geoFenceInfo.fencetype 			= fencetype;
				geoFenceInfo.LngLatPoints 		= LngLatPoints;
				geoFenceInfo.geoFenceName 		= geoFenceName;
				geoFenceInfo.maximumDistance 	= maximumDistance;
				
				geoFenceInfo.isEditable = (p_andruavUnit == null); 
				Me.EVT_andruavUnitGeoFenceUpdated (p_andruavUnit,geoFenceInfo);
			}

			// please move it out side
			this.fn_generateJSONMessage = function (p_senderGroup, p_senderID, p_targetID, p_msgRouting, p_msgID, p_msg)
			{
						
				//prepare json data
				var p_jmsg = {
					ty: p_msgRouting, // MSGRrouting,
					//cm: cm, //cmd, DEPRECATED
					sd: p_senderID, //p_senderID,
					st: 'w', //senderType Web,
					tg: p_targetID, //targetName,
			...
			this.fn_parseJSONMessage = function (JsonMessage)
			{
				...
				var message =
				{
					_ty    		: p_jmsg.ty,                 //command type
					//_cd 		: p_jmsg.cm,                 //main-command DEPRECATED
					_g			: p_jmsg.gr,                 //group name
					_uN			: p_jmsg.sd,                 //sender name
					_mg 		: p_jmsg.ms
				};
				
				if (p_jmsg.hasOwnProperty('mt'))
				{
					message._mt = p_jmsg.mt;
				}
				
				return message;
			};
			
			function prv_parseCommunicationMessage (msg)
			{
						
				var p_jmsg;
				var p_unit = Me.m_andruavUnitList.fn_getUnit(fn_getFullName(msg._g,msg._uN));
				if (p_unit != null)
				{
				...
				switch (msg._mt)
				{
					case CONST_TYPE_AndruavMessage_GPS:
						...
						if (p_unit == null)
						{
							Me.API_requestID(msg._uN);
							return ;
						}
						if (typeof p_jmsg === 'string' || p_jmsg instanceof String)
						{
							// backword compatible
							p_jmsg = fn_json_parse(msg._mg); //Internal message JSON
						}
					...
					case CONST_TYPE_AndruavMessage_CameraList:
					{
						...
						{		
							...
							*/
							...
							if (p_jmsg.R === true)
							{	// this is a reply to request.
								if (v_callbackListeners.hasOwnProperty(CONST_TYPE_AndruavMessage_CameraList) === true)
								{
									v_callbackListeners[CONST_TYPE_AndruavMessage_CameraList](v_session);
									delete v_callbackListeners[CONST_TYPE_AndruavMessage_CameraList];
								}
							}
						}
						else
					...
					case CONST_TYPE_AndruavMessage_CommSignalsStatus:
					{
						...
						if (p_unit == null)
						{
							// p_unit not defined here ... send a request for ID
							Me.API_requestID(p_msg.p_UnitName);
							return ;
						}
					...
					case CONST_TYPE_AndruavMessage_ID:
					{
						...
						var v_trigger_on_swarm_status = false, v_trigger_on_swarm_status2 = false;
						...
						if (p_unit != null)
						{
                            ...
                            if (p_jmsg.hasOwnProperty('C')==true) { 
								//p_unit.m_Telemetry.manualTXBlockedSubAction = p_jmsg.C;
							...
							if (p_jmsg.hasOwnProperty('o') == true) { 
								// SwarmMemberLeaderFormation
                            ...
                            if (p_jmsg.hasOwnProperty('q') == true) { 
							...
							Me.m_andruavUnitList.putUnit(p_unit.fullName(),p_unit);
							window.AndruavLibs.EventEmitter.fn_dispatch(EE_unitUpdated, p_unit);
						}
						else
						{
							p_unit = new AndruavUnitObject();
							p_unit.m_lastActiveTime 			= Date.now(); 
							p_unit.m_IsMe 						= false;
							p_unit.m_IsGCS 						= p_jmsg.GS;
							//p_unit.m_groupName					= msg._g;
							p_unit.m_unitName					= p_jmsg.UD;
							p_unit.partyID						= msg._uN;
							p_unit.Description					= p_jmsg.DS;
							p_unit.m_telemetry_protocol			= p_jmsg.TP;
							p_unit.m_VehicleType				= p_jmsg.VT;
							...
							Me.m_andruavUnitList.Add(p_unit.fullName(),p_unit);
							_fn_onNewUnitAdded(p_unit);
							
							Me.EVT_andruavUnitAdded (p_unit);
						}
						if (v_trigger_on_FCB)	   			Me.EVT_andruavUnitFCBUpdated (p_unit);
						if (v_trigger_on_armed)  			Me.EVT_andruavUnitArmedUpdated (p_unit);
						if (v_trigger_on_flying) 			Me.EVT_andruavUnitFlyingUpdated (p_unit);
						if (v_trigger_on_flightMode)		Me.EVT_andruavUnitFightModeUpdated (p_unit);
						if (v_trigger_on_vehiclechanged)  Me.EVT_andruavUnitVehicleTypeUpdated (p_unit);
					}
					...
					case CONST_TYPE_AndruavMessage_RemoteExecute:
						...
						switch (p_jmsg.C)
						{
							...
							case CONST_RemoteCommand_CLEAR_FENCE_DATA:
								//////var jmsg = fn_json_parse(msg._mg);
								if (p_jmsg.hasOwnProperty('fn')) // fence name
								{
									...
									var keys = Object.keys(GeoLinearFences);
					...
					case CONST_TYPE_AndruavMessage_ExternalGeoFence:
					{
						...
						prv_parseFenceInfo (null,p_jmsg);
					...
					case CONST_TYPE_AndruavMessage_HomeLocation:
					{
						...
						Me.EVT_HomePointChanged (p_unit);
					}
					break;
					...
					case CONST_TYPE_AndruavMessage_DistinationLocation:
					{
						...
						Me.EVT_DistinationPointChanged (p_unit);
					}
					break;
					...
					case CONST_TYPE_AndruavMessage_GeoFence:
					{
						...
						prv_parseFenceInfo (p_unit, msg._mg);
					}	
					break;
					...
					case CONST_TYPE_AndruavMessage_GeoFenceAttachStatus:
					{
						...
						*/
							...
							{
								v_andruavClient.API_requestGeoFences(p_unit,geoFenceAttachStatus.fenceName);
								return ;
							}
							else
							{
								if (fence.Units[p_unit.partyID]==null) 
								{ // not added to this fence .. attach p_unit to fence with missing measures. 
									var geoFenceInfo = {};
									geoFenceInfo.hasValue 				= false; 
									geoFenceInfo.fenceName 			= fence.geoFenceName; 
									geoFenceInfo.m_inZone 				= false; //remember isValid = false
									geoFenceInfo.distance = Number.NaN;
									geoFenceInfo.shouldKeepOutside 	 = fence.shouldKeepOutside;
					...
					case CONST_TYPE_AndruavMessage_GEOFenceHit:
					{
						...
						if (typeof p_jmsg === 'string' || p_jmsg instanceof String)
						{
							...
							fenceName :				p_jmsg.n, 
							m_inZone: 				p_jmsg.z,
							shouldKeepOutside:		(p_jmsg.o==1)
						};
						if (p_jmsg.hasOwnProperty('d')) geoFenceHitInfo.distance = p_jmsg.d; 	else	geoFenceHitInfo.distance = Number.NaN;
					...
					case CONST_TYPE_AndruavMessage_WayPoints:
					{
						...
						{
							if (v_waypointsCache.hasOwnProperty(p_unit.partyID) === false)
							{	
								//! due to disconnection or repeated request this array could be filled of an incomplete previous request.
								//! this value will be reset each time load wp is called.
								v_waypointsCache[p_unit.partyID] = [];
							}

							wayPoint = v_waypointsCache[p_unit.partyID];
						}
						else
						{
							// if this is a full message of the same unit then delete any possible old partial messages -cleaning up-.
							delete v_waypointsCache[p_unit.partyID];
						}
						...
						for (var i = 0; i< numberOfRecords ; ++i)
						{
							if (p_jmsg[i] != null)
							{
								...
								switch (wayPointStep.waypointType)
								{
									...
									case CONST_WayPoint_TYPE_CMissionAction_CONTINUE_AND_CHANGE_ALT:
										wayPointStep.m_Sequence 			= p_jmsg[i].s;
										/**
						...
						if (v_isChunck === WAYPOINT_NO_CHUNK)
						{ // old format message is not a chunk
							Me.EVT_msgFromUnit_WayPoints (p_unit,wayPoint);
						}
						else if (v_isChunck === WAYPOINT_LAST_CHUNK)
						{ // end of chunks
							Me.EVT_msgFromUnit_WayPoints (p_unit,wayPoint);
							delete v_waypointsCache[p_unit.partyID];
						}
					...
					case CONST_TYPE_AndruavMessage_NAV_INFO:
					{
						...
						*/	
						...
						Me.EVT_msgFromUnit_NavInfo (p_unit);
					}
					break;
			...
			{
				Me.socketStatus = status;
				
				if (status== CONST_SOCKET_STATUS_CONNECTED)
				{
					v_andruavClient.API_addMe2(); //(v_andruavClient.groupname,v_andruavClient.unitID);
				}
				...
				if (status == CONST_SOCKET_STATUS_REGISTERED)
				{
					...
					Me.API_sendID();  // send now important
				...
				Me.fn_onSocketStatus(status,SOCKET_STATUS[status-1]);
			};

			function prv_parseSystemMessage (msg)
			{
				if (msg._mt == CONST_TYPE_AndruavSystem_ConnectedCommServer)
				{
					if (msg._mg.s.indexOf('OK:connected')!=-1)
					{
						setSocketStatus(CONST_SOCKET_STATUS_CONNECTED);
						setSocketStatus(CONST_SOCKET_STATUS_REGISTERED);
				...
				// if ((msg._cd == 'add') || (msg._cd == 'addd'))
				// {
				// 	if (msg._mg.indexOf('OK:add')!=-1)
				// 	{
				// 		setSocketStatus(CONST_SOCKET_STATUS_REGISTERED);
				// 	}
				// 	else
				...
				if (msg._mt === CONST_TYPE_AndruavSystem_LogoutCommServer)
				{
					if (msg._mg.s.indexOf('OK:del')!=-1)
					{
						setSocketStatus(CONST_SOCKET_STATUS_FREASH);
						Me.EVT_onDeleted();
					}
					else
					{
			...
			function prv_extractBinaryPacket (evt)
			{
				...
				var andruavCMD;
				var v_internalCommandIndexByteBased;
				var p_jmsg;
				var data;
				var v_unit;
				var byteLength;
				 ...
				 */
				function prv_parseMavlinkMessage (p_unit,p_mavlinkPacket)
				{
					let p_mavlinkProcessor = new MAVLink20Processor(null, 42, 150);
					var p_mavlinkMessages = p_mavlinkProcessor.parseBuffer(new Uint8Array(p_mavlinkPacket));
				...
				function prv_parseBinaryAndruavMessage ()
				{
					...
					switch (andruavCMD.mt)
					{
						...
						case CONST_TYPE_AndruavMessage_LightTelemetry:
						{
							...
							prv_parseMavlinkMessage (v_unit,v_andruavMessage.data);
							Me.EVT_OnTelemetryIn (v_unit,v_andruavMessage.data);
						}	
						break;			
						...
						// case CONST_TYPE_AndruavMessage_Telemetry:
						// {
						// 	var out = prv_extractString (data,v_internalCommandIndexByteBased,byteLength);
						// 	out.json = fn_json_parse(out.text);
						// 	var andruavMessage = {
						// 		'src': out.json.src,
						// 		'data': data.buffer.slice(out.nextIndex) //prv_extractBinary(data,out.nextIndex,byteLength)	
						// 	};
						// 	Me.EVT_OnTelemetryIn (v_unit,andruavMessage);		
						// }
						// break;
						...
						case CONST_TYPE_AndruavMessage_IMG:
						{
							...
							Me.EVT_msgFromUnit_IMG (v_unit,
								v_andruavMessage.img,
								v_andruavMessage.des,
								v_andruavMessage.lat,
								v_andruavMessage.lng,
								v_andruavMessage.prv,
								v_andruavMessage.tim,
								v_andruavMessage.alt,
								v_andruavMessage.spd,
								v_andruavMessage.ber,
								v_andruavMessage.acc
								);
				...
				reader.onload = function(event) {
					...
					v_unit = Me.m_andruavUnitList.fn_getUnit(fn_getFullName(p_jmsg._g,p_jmsg._uN));
					if ((v_unit == null) && (p_jmsg._mt!=CONST_TYPE_AndruavMessage_ID)) 
					{
						Me.API_requestID(p_jmsg._uN);
						return ;
					}	
					v_internalCommandIndexByteBased = out.nextIndex;
										
					prv_parseBinaryAndruavMessage ();
				};
			...
			{		
				...
				{ 
					if (typeof evt.data == "string")
					{
						...
						{
							case CMDTYPE_SYS: 
								prv_parseSystemMessage(p_jmsg);
								break;
								
							case CMD_COMM_GROUP:
							case CMD_COMM_INDIVIDUAL:
								prv_parseCommunicationMessage(p_jmsg);
								break;
						}
				...
				{ 
					setSocketStatus(CONST_SOCKET_STATUS_DISCONNECTED);
					Me.EVT_onClose();
				};
				
				this.ws.onerror = function (err)
				{
					setSocketStatus(CONST_SOCKET_STATUS_ERROR);
					Me.EVT_onError (err);
				};
			}
			else
    ...
    global.AndruavClient = new AndruavClient();
	
		//AndruavLibs.AndruavClient = new AndruavClient ();
