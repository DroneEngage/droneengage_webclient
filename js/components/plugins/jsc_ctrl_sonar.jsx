export class CLSS_CTRL_SONAR    extends React.Component {
    ...
    render() {
        ...
        var v_sonar_lvl_class = ' text-warning';
        var  v_sonar_buttom =[];
        
        if ((c_sonar.m_is_initialized===true) && (c_sonar.m_is_paused===false))
        {
            v_sonar_buttom.push(<p className="text-success">Sonar is Active <span className="text-success">{c_sonar.m_log_filename}</span></p>);
        }
        else
        {
            v_sonar_buttom.push(<p className="text-danger">Sonar is Inactive <span className="text-light">{c_sonar.m_log_filename}</span></p>);
        }



        return(
            <div key='sonar_1' id='sonar_1' className= 'row al_l  css_margin_zero '>
