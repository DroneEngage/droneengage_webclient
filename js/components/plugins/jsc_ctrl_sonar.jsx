export class CLSS_CTRL_SONAR    extends React.Component {
    ...
    render() {
        ...
        {
            v_sonar_buttom.push(<p key={v_andruavUnit.partyID + 'sonar_ctrl_sb1'}  className="text-success">Sonar is Active <span className="text-success">{c_sonar.m_log_filename}</span></p>);
        }
        else
        {
            v_sonar_buttom.push(<p key={v_andruavUnit.partyID + 'sonar_ctrl_sb2'} className="text-danger">Sonar is Inactive <span className="text-light">{c_sonar.m_log_filename}</span></p>);
        }

        
        return(
            <div key={v_andruavUnit.partyID + 'sonar_ctrl'} id='sonar_1' className= 'row al_l  css_margin_zero '>
                <div key={v_andruavUnit.partyID + 'sonar_ctrl_1'} className = 'row al_l css_margin_zero d-flex '>
                    <div key={v_andruavUnit.partyID + 'sonar_ctrl_11'}  className= 'col-12 user-select-none '>
                        {v_sonar_buttom}
                        <div key={v_andruavUnit.partyID + 'sonar_ctrl_111'} id="main_btn_group"  role="group" aria-label="Basic example">
