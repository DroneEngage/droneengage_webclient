...
Generated from: all.xml,ardupilotmega.xml,ASLUAV.xml,common.xml,development.xml,icarous.xml,minimal.xml,python_array_test.xml,standard.xml,test.xml,ualberta.xml,uAvionix.xml,storm32.xml,AVSSUAS.xml
...
mavlink20.MAV_CMD_PRS_SET_CHARGE_MV = 60073 // AVSS defined command. Set the threshold to charge from outside in
                        // millivolts
mavlink20.MAV_CMD_PRS_GET_CHARGE_MV = 60074 // AVSS defined command. Get the threshold to charge from outside in
                        // millivolts.
mavlink20.MAV_CMD_PRS_SET_TIMEOUT = 60075 // AVSS defined command. Set the timeout between FTS request and
                        // deploying the chute.
mavlink20.MAV_CMD_PRS_GET_TIMEOUT = 60076 // AVSS defined command. Get the timeout between FTS request and
                        // deploying the chute.
mavlink20.MAV_CMD_PRS_SET_FTS_CONNECT = 60077 // AVSS defined command. Set up the PRS to connect to the drone..
mavlink20.MAV_CMD_PRS_GET_FTS_CONNECT = 60078 // AVSS defined command. Get the connection status of PRS and drone.
mavlink20.MAV_CMD_ENUM_END = 60079 // 
...
mavlink20.MAV_QSHOT_MODE_ENUM_END = 9 // 
...
can use this checksum to determine if it has a matching plan
definition).         This message must be broadcast following any
change to a plan (immediately after the MISSION_ACK that completes the
plan upload sequence).         It may also be requested using
MAV_CMD_REQUEST_MESSAGE, where param 2 indicates the plan type for
which the hash is required.         The checksum must be calculated on
the autopilot, but may also be calculated by the GCS.         The
checksum uses the same CRC32 algorithm as MAVLink FTP
(https://mavlink.io/en/services/ftp.html#crc32-implementation).
It is run over each item in the plan in seq order (excluding the home
location if present in the plan), and covers the following fields (in
order):         frame, command, autocontinue, param1, param2, param3,
...
can be set to NaN according to use case.
                ...
                q                         : Quaternion components, w, x, y, z (1 0 0 0 is the null-rotation, the frame is determined by the STORM32_GIMBAL_DEVICE_FLAGS_YAW_ABSOLUTE flag, NaN to be ignored). (float)
...
never to react to this message.
                ...
                q                         : Quaternion components, w, x, y, z (1 0 0 0 is the null-rotation, the frame is determined by the GIMBAL_MANAGER_FLAGS_ABSOLUTE_YAW flag, NaN to be ignored). (float)
                angular_velocity_x        : X component of angular velocity (positive: roll to the right, NaN to be ignored). (float)
                angular_velocity_y        : Y component of angular velocity (positive: tilt up, NaN to be ignored). (float)
                angular_velocity_z        : Z component of angular velocity (positive: pan to the right, the frame is determined by the STORM32_GIMBAL_DEVICE_FLAGS_YAW_ABSOLUTE flag, NaN to be ignored). (float)
...
AVSS PRS system status.
                ...
                battery_status            : Estimated battery run-time without a remote connection and PRS battery voltage (uint16_t)
                error_status              : PRS error statuses (uint32_t)
                change_status             : PRS battery change statuses (uint8_t)
                time_boot_ms              : Time since PRS system boot (uint32_t)

*/
mavlink20.messages.avss_prs_sys_status = function(arm_status, battery_status, error_status, change_status, time_boot_ms) {

    this.format = '<IIHBB';
    this.id = mavlink20.MAVLINK_MSG_ID_AVSS_PRS_SYS_STATUS;
    this.order_map = [3, 2, 0, 4, 1];
    this.crc_extra = 153;
    this.name = 'AVSS_PRS_SYS_STATUS';

    this.fieldnames = ['arm_status', 'battery_status', 'error_status', 'change_status', 'time_boot_ms'];
...
mavlink20.messages.avss_prs_sys_status.prototype.pack = function(mav) {
    return mavlink20.message.prototype.pack.call(this, mav, this.crc_extra, jspack.Pack(this.format, [ this.error_status, this.time_boot_ms, this.battery_status, this.arm_status, this.change_status]));
}
...
mavlink20.map = {
        ...
        60050: { format: '<IIHBB', type: mavlink20.messages.avss_prs_sys_status, order_map: [3, 2, 0, 4, 1], crc_extra: 153 },
}
...
MAVLink20Processor.prototype.pushBuffer = function(data) {
    if(data) {
        ...
        c.set (data);
        this.buf = c;
        //this.buf = data; //new Uint8Array([this.buf, data]); //mhefny
